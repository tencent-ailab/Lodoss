...
def add_model_specific_args(parser):
    ...
    parser.add_argument("--save_summary", action='store_true', help="Set to save predicted summary")

    # **************** Parameters for model **************** #
    parser.add_argument("--n_transformer_layer", type=int, default=2,
                        help="number of sentence-level transformer layers")
    parser.add_argument("--n_transformer_head", type=int, default=8,
                        help="number of multi-heads in the Transformer layer")
    parser.add_argument("--dropout_transformer", type=float, default=0.2,
    ...
    parser.add_argument("--attention_window", type=int,
                        default=512, help="Local attention window size for Longformer; set the same window size")
    parser.add_argument("--increasing_window_size", action="store_true",
                        help="Apply increasing attention window size from lower to higher layer")
    ...
    parser.add_argument("--is_seg", action="store_true", help="Set for SS and SSD")
    parser.add_argument("--is_dpp", action="store_true", help="Set for SSD")
    parser.add_argument("--dpp_weight", type=float, default=0.01, help="DPP loss weight for summarization")
    parser.add_argument("--seg_label_pos", type=int, default=-1,
                        choices=[-1, 0], help="segmentation label position; the first or last sentence")
    ...
    parser.add_argument("--data_set", type=str, default="pubmed", choices=["pubmed", "arxiv"])
    ...
    parser.add_argument("--cache_eval_dir", type=str,
                        default='/huggingface_evaluation_metric')
